...
class TestConstants:
    ...
    def get_error_message(cls, error_type, **kwargs):
        ...
        if error_type == "course_not_found":
            return cls.ERROR_COURSE_NOT_FOUND.format(
                id=kwargs.get("id", cls.NON_EXISTENT_COURSE_ID)
            )
        elif error_type == "fetch_all_courses":
            return cls.ERROR_FETCH_ALL_COURSES.format(
                error=kwargs.get("error", cls.ERROR_DATABASE)
            )
        return ""
